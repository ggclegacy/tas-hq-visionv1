...
export function validateManifest(input: unknown): ValidationResult {
  ...
  if (manifest.shots) {
    ...
    sortedShots.forEach((shot, shotIndex) => {
      ...
      if (
        shot.overlapMs !== undefined &&
        (shot.overlapMs < 500 || shot.overlapMs > 1_600)
      )
        issue(
          `${path}.overlapMs`,
          "invalid_overlap",
          "Shot overlap must be between 500ms and 1600ms.",
        );
      if (
        shot.fullLegibilityMs !== undefined &&
        (shot.fullLegibilityMs < shot.startMs ||
          shot.fullLegibilityMs - shot.startMs > 900)
      )
        issue(
          `${path}.fullLegibilityMs`,
          "reveal_budget",
          "Primary copy must become fully legible within 900ms.",
        );
