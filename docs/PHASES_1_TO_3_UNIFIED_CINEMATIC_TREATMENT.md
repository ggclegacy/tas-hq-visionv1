...
The GAC shot succeeds when: activity is visible on frame one; the emblem is identifiable within 1.2 seconds; copy is fully legible within 900ms; obsidian/graphite depth spans foreground/midground/background; green is selective; champagne gold reads as metal rather than mustard; camera/light remain alive through the hold; emblem rim/specular/parallax are deterministic; the architecture/emblem carrier motivates dedication; portrait and desktop retain confident emblem scale; reduced motion retains lighting and hierarchy; all tiers preserve meaning; direct/backward seek and replay match; no new loop or dependency is introduced.

## Shared visual system

The persistent stage supplies camera transforms, architecture, atmosphere, subject, typography, and foreground. Shots author focal state, layers, light, viewport profiles, overlap carrier, reveal point, emblem variant, tiers, and reduced motion. Adjacent formation is resolved purely from Director time. Scene 1 uses DOM/CSS because bounded gradients, masks, transforms, and the protected raster emblem meet the requirement without WebGL risk.
